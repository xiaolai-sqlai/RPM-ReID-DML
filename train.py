...
from network.adaloss import AdaLoss
...
parser.add_argument('--decoder', type=str, default="rpm_max", help="pcb_avg,pcb_max,rpm_avg,rpm_max")
parser.add_argument('--num-part', type=int, default=2)
parser.add_argument('--num-stripe', type=int, default=1)
parser.add_argument('--feat-num', type=int, default=0)
parser.add_argument('--use-global', type=str2bool, default=True)
parser.add_argument('--ada-gamma', type=float, default=0.0)
...
def main(net, batch_size, epochs, opt):
    ...
    criterion1 = AdaLoss(num_classes, opt.ada_gamma).cuda()
        ...
        for data, label in train_data:
            ...
            if opt.amp:
                with torch.cuda.amp.autocast():
                    ...
                    weight = opt.num_part
                    losses.append(criterion1(outputs, label_list))
                    if opt.use_global:
                        weight += 2
...
if __name__ == '__main__':
    ...
    if "rpm" in opt.decoder:
        net = RPM(decoder=opt.decoder, num_classes=num_classes, num_part=opt.num_part, num_stripe=opt.num_stripe, feat_num=opt.feat_num, std=opt.std, net=opt.net, erasing=opt.erasing, h=opt.img_height//16, w=opt.img_width//16, use_global=opt.use_global)
    elif "pcb" in opt.decoder:
        net = PCB(decoder=opt.decoder, num_classes=num_classes, num_part=opt.num_part, num_stripe=opt.num_stripe, feat_num=opt.feat_num, std=opt.std, net=opt.net, erasing=opt.erasing, h=opt.img_height//16, w=opt.img_width//16, use_global=opt.use_global)
    ...
    for _ in range(5):
        os.system("python test_{}.py --net {} --decoder {} --img-height {} --img-width {} --batch-size 32 --gpus {} --dataset-root {} --num-part {} --feat-num {} --use-global {}".format(opt.dataset, opt.net, opt.decoder, opt.img_height, opt.img_width, opt.gpus, opt.dataset_root, opt.num_part, opt.feat_num, opt.use_global))

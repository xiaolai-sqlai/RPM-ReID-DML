...
class RPM(nn.Module):
    def __init__(self, decoder, num_classes=751, num_part=2, num_stripe=1, feat_num=0, std=0.1, net="regnet_y_1_6gf", erasing=0.0, h=384, w=128, use_global=True):
        ...
        embed_dim = 1024 // num_part
        if embed_dim * num_part == 1024:
            embed_list = [embed_dim] * num_part
        else:
            embed_list = [embed_dim + 1] * (num_part - 1)
            embed_list.append(1024 - sum(embed_list))
        ...
        for i in range(self.num_part):
            if "max" in decoder:
                ...
                self.pool_list.append(GlobalAvgPool2d(p=1))
            if self.feat_num == 0:
                feat_num = embed_list[i]
                feat = nn.Identity()
            else:
                feat = nn.Linear(embed_list[i], feat_num, bias=False)
                init.kaiming_normal_(feat.weight)
            self.feat_list.append(feat)
            bn = nn.BatchNorm1d(feat_num)
            init.normal_(bn.weight, mean=1.0, std=std)
        ...
        print(self.learnable_masks)

    def init_mask(self):
        if self.num_stripe == 0:
            return torch.ones(self.num_part, 1, 1, self.h, self.w)
        learnable_masks = []
        for i in range(self.num_part):
            chunks = self.num_part + self.num_stripe - 1
            learnable_mask = torch.zeros(1, 1, 1, self.h, self.w)
            masks = torch.chunk(learnable_mask, dim=3, chunks=chunks)
            for j in range(self.num_stripe):
                masks[i+j][:] = 1
            learnable_masks.append(torch.cat(masks, dim=3))

        return torch.cat(learnable_masks, dim=0)
    ...
    def forward(self, x):
        ...
        if self.use_global:
            x_chunk = [x1, x1]
        for i, part in enumerate(torch.chunk(x2, dim=1, chunks=self.num_part)):
            x_chunk.append(part * self.learnable_masks[i])
        ...
        return bn_list, self.learnable_masks

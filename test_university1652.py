...
def extract_feature(net, dataloaders):
    ...
    for img, _ in dataloaders:
        ...
        if count % 100 == 0:
            print(count)
        if opt.use_global:
            ff = np.zeros((n, opt.feat_num*(opt.num_part+2)), dtype=np.float32)
        else:
            ff = np.zeros((n, opt.feat_num*opt.num_part), dtype=np.float32)
        for i in range(2):
            if(i==1):
...
if __name__ == '__main__':
    ...
    for query_name, gallery_name in (["query_satellite", "gallery_drone"], ["query_drone", "gallery_satellite"]):
        query_set = []
        query_label = []
        query_cam = []
        for label in os.listdir(osp.join(data_dir, "test", query_name)):
            for name in os.listdir(osp.join(data_dir, "test", query_name, label)):
                query_set.append([osp.join(data_dir, "test", query_name, label, name), label])
        ...
        test_label = []
        test_cam = []
        for label in os.listdir(osp.join(data_dir, "test", gallery_name)):
            for name in os.listdir(osp.join(data_dir, "test", gallery_name, label)):
                test_set.append([osp.join(data_dir, "test", gallery_name, label, name), label])
        ...
        if "rpm" in opt.decoder:
            net = RPM(decoder=opt.decoder, num_classes=701, num_part=opt.num_part, feat_num=opt.feat_num, net=opt.net, h=opt.img_height//16, w=opt.img_width//16, use_global=opt.use_global)
        elif "pcb" in opt.decoder:
            net = PCB(decoder=opt.decoder, num_classes=701, num_part=opt.num_part, feat_num=opt.feat_num, net=opt.net, h=opt.img_height//16, w=opt.img_width//16, use_global=opt.use_global)
